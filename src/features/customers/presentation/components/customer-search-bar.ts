...
export class CustomerSearchBar extends LitElement {
  ...
  protected render() {
    return html`
      <div class="bg-surface-panel/40 border border-white/5 rounded-2xl p-6 mb-8 shadow-sm">
        ...
        <div class="flex flex-wrap gap-3">
          <button class="flex items-center gap-2 px-4 py-2 rounded-full bg-brand/15 border border-brand/30 text-brand-light text-sm font-medium transition-colors">
             <app-icon .icon=${Search} .size=${14}></app-icon>
             All
          </button>
          
          <button class="flex items-center gap-2 px-4 py-2 rounded-full bg-surface-card border border-white/5 text-text-secondary text-sm font-medium hover:bg-white/5 transition-colors">
