...
export class OrderListTable extends LitElement {
      ...
      <div class="bg-surface-panel/40 border border-white/5 rounded-2xl overflow-hidden shadow-sm">
          ...
          <table class="w-full text-left border-collapse">
            <thead>
              <tr class="border-b border-white/5">
                ...
                <th class="px-6 py-4 text-xs font-bold text-text-dim uppercase tracking-wider">Payment</th>
            ...
            <tbody class="divide-y divide-white/5">
              ${FAKE_ORDERS.map(order => this.renderRow(order))}
            </tbody>
          </table>
        </div>
        ...
        <div class="px-6 py-4 flex items-center justify-between border-t border-white/5 bg-surface-card/30">
          <div class="text-sm text-text-dim">
            Showing <span class="text-text-primary font-medium">1–4</span> of <span class="text-text-primary font-medium">4</span>
          </div>
  ...
  private renderRow(order: OrderData) {
    return html`
      <tr class="hover:bg-white/[0.02] transition-colors group">
        ...
        <td class="px-6 py-4 text-sm text-text-secondary font-medium">${order.customerName}</td>
        ...
        <td class="px-6 py-4 text-sm text-text-secondary">${order.payment}</td>
        <td class="px-6 py-4">
  ...
  private renderDeliveryBadge(status: string) {
    ...
    const config = configs[status as keyof typeof configs] || 'bg-surface-card text-text-dim border-white/5';
